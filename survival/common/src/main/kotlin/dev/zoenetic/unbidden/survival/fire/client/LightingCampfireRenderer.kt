package dev.zoenetic.unbidden.survival.fire.client

import com.mojang.blaze3d.vertex.PoseStack
import com.mojang.math.Axis
import dev.zoenetic.unbidden.survival.fire.ClientFireAttempt
...
public interface LightingCampfireState {
    public fun `unbidden$getLightingCampfire`(): Boolean
    public fun `unbidden$setLightingCampfire`(state: Boolean)
}
...
public object LightingCampfireRenderer {
    ...
    public fun capture(entity: LivingEntity, state: LivingEntityRenderState) {
        ...
        (state as LightingCampfireState).`unbidden$setLightingCampfire`(lighting)
    }
    ...
    public fun applyToModel(
        ...
        if (!(state as LightingCampfireState).`unbidden$getLightingCampfire`()) return
