package dev.zoenetic.unbidden.survival.vitals.client

import com.mojang.blaze3d.vertex.PoseStack
import com.mojang.math.Axis
import dev.zoenetic.unbidden.survival.Survival
import dev.zoenetic.unbidden.survival.vitals.shiverIntensity
...
public interface ShiverState {
    public fun `unbidden$getShiver`(): Double
    public fun `unbidden$setShiver`(shiver: Double)
}
...
public object ShiverRenderer {
    ...
    public fun capture(entity: LivingEntity, state: LivingEntityRenderState) {
        ...
        (state as ShiverState).`unbidden$setShiver`(intensity)
    }

    public fun applyToBodyRotation(bodyRot: Float, state: LivingEntityRenderState): Float {
        val intensity = (state as ShiverState).`unbidden$getShiver`()
